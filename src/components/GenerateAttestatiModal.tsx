...
const GenerateAttestatiModal: React.FC<GenerateAttestatiModalProps> = ({
  ...
  const fetchScheduledCourses = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:4000/schedules-with-attestati');
                  ...
                  {scheduledCourses
                    ...
                    .map((course) => (
                      <option key={course.id} value={course.id}>
                        {course.course.title} - {new Date(course.start_date).toLocaleDateString('it-IT')} - {course.location}
                      </option>
                    ))}
                </select>
              </div>
